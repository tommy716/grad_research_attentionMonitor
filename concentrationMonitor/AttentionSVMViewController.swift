...
class AttentionSVMViewController: UIViewController {
    ...
    func rectColor() -> CGColor {
        ...
        // MARK: ここで集中度推測
        let attentionAnalyzer = attention_svm()
        ...
        if self.lastConcentrationTime != nil {
            print(Date().timeIntervalSince(self.lastConcentrationTime!))
            self.concentrationTime += Date().timeIntervalSince(self.lastConcentrationTime!)
            self.lastConcentrationTime = nil
        }
...
extension AttentionSVMViewController: AVCaptureVideoDataOutputSampleBufferDelegate {
